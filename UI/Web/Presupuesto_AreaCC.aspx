<%@ Page Title="" Language="C#" MasterPageFile="~/Web/Menu.Master" AutoEventWireup="true" CodeBehind="Presupuesto_AreaCC.aspx.cs" Inherits="UI.Web.Presupuesto_AreaCC" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <script type="text/javascript">
        ...
        var isDetailRowExpanded = false;
        var buttonID;

        function expandDetailRow(mainASPxGridView, clickedButtonID, focusedRowIndex) {
            buttonID = clickedButtonID;
            mainASPxGridView.ExpandDetailRow(focusedRowIndex);
        }

        function expandCollapseDetailRow(focusedRowIndex, eventArgs) {
            if (!isDetailRowExpanded) {
                isDetailRowExpanded = true;
                expandDetailRow(mainASPxGridView, eventArgs.buttonID, focusedRowIndex);
            }
            else {
                if (eventArgs.buttonID != buttonID)
                    expandDetailRow(mainASPxGridView, eventArgs.buttonID, focusedRowIndex);
                else
                {
                    isDetailRowExpanded = false;
                    mainASPxGridView.CollapseDetailRow(focusedRowIndex);
                }
            }
        ...
        function setDetailRowGridViewVisible(mainASPxGridView, clickedButtonID) {
            if (buttonID == clickedButtonID)
                mainASPxGridView.SetVisible(true);
            else
                mainASPxGridView.SetVisible(false);
        }

        function onFirstDetailGridViewInit(s, e) {
            setDetailRowGridViewVisible(s, "firstDetailButton");
        }

        function onSecondDetailGridViewInit(s, e) {
            setDetailRowGridViewVisible(s, "secondDetailButton");
        }
    </script>
...
;Password=P0rta1R3p.2766$" ProviderName="System.Data.SqlClient" SelectCommand="SELECT AREA,DESCRIPCION
...
WHERE AREA = @AREA" InsertCommand="INSERT INTO PORTAL.[Presupuesto].[AREA]
           (DESCRIPCION)
     VALUES (@DESCRIPCION)" UpdateCommand="Update [PORTAL].[Presupuesto].[AREA]
...
WHERE AREA = -1 AND CUENTA = @CUENTA" InsertCommand="INSERT INTO  portal.[Presupuesto].[AREA_CUENTA]
           (AREA, CUENTA)
...
;Password=P0rta1R3p.2766$" ProviderName="System.Data.SqlClient" SelectCommand="SELECT CENTRO_COSTO, CENTRO_COSTO + ' ' +DESCRIPCION DESCRIPCION
  ...
  order by CENTRO_COSTO">
    ...
    <asp:SqlDataSource ID="SQLArea_Centro" runat="server" ConnectionString="Data Source=10.144.10.10;Initial Catalog=me;Persist Security Info=True;User ID=PortalRep
;Password=P0rta1R3p.2766$" ProviderName="System.Data.SqlClient" SelectCommand="SELECT CENTRO_COSTO, AREA
  FROM portal.[Presupuesto].[AREA_CENTRO]
WHERE AREA = @AREA
  order by 1" DeleteCommand="DELETE  portal.[Presupuesto].[AREA_CENTRO]
WHERE AREA = -1 AND CENTRO_COSTO = @CENTRO_COSTO" InsertCommand="INSERT INTO  portal.[Presupuesto].[AREA_CENTRO]
           (AREA, CENTRO_COSTO)
     VALUES (@AREA, @CENTRO_COSTO)" UpdateCommand="Update  portal.[Presupuesto].[AREA_CENTRO]
...
WHERE AREA = @AREA">
    ...
    <br />
    <dx:ASPxGridView ID="ASPxGridView1" ClientInstanceName="mainASPxGridView" runat="server" AutoGenerateColumns="False" DataSourceID="SQLArea" Theme="SoftOrange" Width="427px" KeyFieldName="AREA">
        <ClientSideEvents CustomButtonClick="function(s, e) { expandCollapseDetailRow(s.GetFocusedRowIndex(), e) }" FocusedRowChanged="onFocusedRowChanged" />
...
<HeaderFilter MinHeight="140px"></HeaderFilter>
        ...
        <Columns>
            ...
            <dx:GridViewCommandColumn>
                    <CustomButtons>
                        <dx:GridViewCommandColumnCustomButton ID="firstDetailButton" Text="Cuenta Contable" />
                        <dx:GridViewCommandColumnCustomButton ID="secondDetailButton" Text="Centro Costo" />
                    </CustomButtons>
                </dx:GridViewCommandColumn>
            ...
            <dx:GridViewDataTextColumn FieldName="DESCRIPCION" LoadReadOnlyValueFromDataModel="True" VisibleIndex="2">
              ...
              <Templates>
					<DetailRow>
                         <asp:HiddenField ID="detailHiddenField" runat="server" Value='<%# Eval("AREA") %>' />
						<dx:ASPxGridView ID="firstDetailSqlDataSource" runat="server" AutoGenerateColumns="False" DataSourceID="SQLArea_Cuenta"
                            ...
                            <ClientSideEvents Init="onFirstDetailGridViewInit" />
							<Columns>
                        ...
                        <dx:ASPxGridView ID="secondDetailGridView" runat="server" KeyFieldName="CENTRO_COSTO" DataSourceID="SQLArea_Centro"
                            OnBeforePerformDataSelect="SdetailGrid_BeforePerformDataSelect" OnRowDeleting="SGrid_RowDeleting" OnRowInserting="SGrid_RowInserting" OnRowUpdating="SGrid_RowUpdating" Theme="SoftOrange" Width="420px" >
                        <ClientSideEvents Init="onSecondDetailGridViewInit" />
                        <Columns>
								<dx:GridViewCommandColumn ShowNewButton="true" ShowEditButton="false" ShowDeleteButton="true" VisibleIndex="0">
								</dx:GridViewCommandColumn>
                                <dx:GridViewDataComboBoxColumn FieldName="CENTRO_COSTO" LoadReadOnlyValueFromDataModel="True" VisibleIndex="1">
                                  <PropertiesComboBox DataSourceID="SQLCentroCosto" TextField="DESCRIPCION" ValueField="CENTRO_COSTO" Width ="400px">
                                  </PropertiesComboBox>
                                </dx:GridViewDataComboBoxColumn>
								<dx:GridViewDataTextColumn FieldName="AREA" VisibleIndex="2" Visible="false">
								</dx:GridViewDataTextColumn>
